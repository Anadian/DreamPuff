...
FLAG_PREDEFINED_MACROS?=-DGIT_COMMIT=$(GIT_COMMIT)
...
ifeq ($(origin BUILD_CONDITION_WINDOWS_SDL2),undefined)
BUILD_CONDITION_WINDOWS_SDL2::=0
ifeq ($(OS),Windows_NT)
ifeq ($(MIDDLEWARE),SDL2)
BUILD_CONDITION_WINDOWS_SDL2::=1
BUILD_CONDITION=Windows_SDL2
LINK_FLAGS?= -lSDL2 -lSDL2_image -lSDL2_mixer -lSDL2_ttf 
endif #($(MIDDLEWARE),SDL2)
endif #($(OS),Windows_NT)
endif #($(origin BUILD_CONDITION_WINDOWS_SDL2),undefined)
...
FLAGS=$(FLAG_PREDEFINED_MACROS) $(FLAG_INCLUDE_SEARCH_DIRECTORIES) $(FLAG_STATIC_LIBRARY_SEARCH_DIRECTORIES) $(LINK_FLAGS) $(VERBOSE_FLAGS)
